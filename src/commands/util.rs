...
impl Client {
    ...
    pub fn load_login(&mut self) -> anyhow::Result<()> {
        if self.test_mode {
    ...
    pub fn try_login(&mut self, username: String, password: String) -> anyhow::Result<String> {
        if self.test_mode {
    ...
    pub fn list_courses(&mut self) -> anyhow::Result<Vec<Course>> {
        if self.test_mode {
            return Ok(vec![
                Course {
